...
export default function Footer() {
  return (
    <footer id="conditions" className="relative overflow-hidden border-t border-slate-800 bg-slate-950/95 pt-16 pb-8 text-slate-300">
      <div className="pointer-events-none absolute inset-0 bg-[radial-gradient(ellipse_at_top,_rgba(59,130,246,0.08),_transparent_60%)]" />
          ...
          <ScrollReveal className="md:col-span-2">
            ...
            <div className="mb-5 space-y-2 text-sm text-slate-400">
              ...
              <div className="flex items-center gap-2">
                ...
                <a href="tel:+918151912525" className="hover:text-blue-300">+91-8151912525</a>
              </div>
              <div className="flex items-center gap-2">
                <Mail className="h-4 w-4 text-blue-400" />
                <a href="mailto:physiofix2525@gmail.com" className="hover:text-blue-300">physiofix2525@gmail.com</a>
              </div>
            </div>
          ...
          {columns.map((col, i) => (
            <ScrollReveal key={col.title} delay={i * 0.08}>
              ...
              <ul className="space-y-2.5">
                {col.links.map((link) => (
                  <li key={link.label}>
                    <Link href={link.href} className="text-sm text-slate-400 transition hover:text-blue-300">{link.label}</Link>
                  </li>
                ))}
              </ul>
            </ScrollReveal>
