...
const specialties: { name: string; icon: LucideIcon; bg: string; border: string; color: string; detail: string; active?: boolean }[] = [
  ...
  { name: "Posture", icon: Wind, bg: "from-teal-500/12 to-cyan-400/8", border: "border-teal-200/70", color: "text-teal-600", detail: "Alignment and breathing" },
];
        ...
        <div className="section-shell px-6 py-8 md:px-8 lg:px-10">
          <ScrollReveal className="mb-8 flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
            ...
            <div className="flex flex-wrap gap-2">
              {categories.map((item) => (
                <button key={item.label} className={`rounded-full px-4 py-2 text-sm font-semibold transition-all duration-300 ${item.active ? "bg-gradient-to-r from-teal-600 to-blue-600 text-white shadow-md" : "bg-slate-100 text-slate-600 hover:bg-slate-200"}`}>
                  {item.label}
                </button>
              ))}
            </div>
            ...
            {specialties.map((specialty) => {
              ...
              return (
                <StaggerItem key={specialty.name}>
                  <TiltCard className={`rounded-[1.25rem] border bg-gradient-to-br ${specialty.bg} p-4 text-left ${specialty.border} ${specialty.active ? "ring-2 ring-teal-400/60" : ""}`}>
          ...
          <ScrollReveal className="mt-8 rounded-[1.25rem] border border-teal-100/60 bg-gradient-to-br from-teal-50/50 via-white to-blue-50/40 px-4 py-4">
            <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
              ...
              <Link href="/services" className="inline-flex items-center gap-2 text-sm font-semibold text-slate-700 transition hover:text-teal-600">
              ...
              {treatmentList.map((item) => {
                ...
                return (
                  <StaggerItem key={item.title}>
                    <Link
                      href={item.href}
                      className="group flex h-full rounded-[1.15rem] border border-slate-200/80 bg-white/90 p-4 transition-all duration-300 hover:-translate-y-0.5 hover:border-teal-400 hover:shadow-[0_12px_40px_rgba(20,184,166,0.12)]"
                    >
                      <div className="flex w-full items-start justify-between gap-3">
                        <div>
                          <div className="mb-2 flex h-8 w-8 items-center justify-center rounded-xl bg-teal-50 text-teal-600 transition group-hover:bg-teal-100">
                            <Icon className="h-4 w-4" />
                          </div>
                          <p className="text-sm font-semibold text-slate-900">{item.title}</p>
                          <p className="mt-2 text-sm leading-6 text-slate-600">{item.description}</p>
                        </div>
                        <span className="mt-0.5 rounded-full bg-teal-50 p-2 text-teal-600 transition group-hover:bg-teal-100">
